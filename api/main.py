...
@app.get('/subscription/checkout', summary='Get Paddle checkout URL')
async def get_checkout(
    user_id: str = Depends(get_current_user_id),
    db: AsyncSession = Depends(get_db),
):
    """Returns a Paddle checkout URL for the $23/month subscription."""
    user = await get_user_by_id(db, user_id)
    if not user:
        raise HTTPException(status_code=404, detail='User not found')
    from payments.paddle import get_checkout_url
    url = get_checkout_url(user.email, user_id)
    return {'checkout_url': url}


@app.get('/subscription/status', summary='Check subscription status')
async def subscription_status(
    user_id: str = Depends(get_current_user_id),
    db: AsyncSession = Depends(get_db),
):
    """Returns whether the user has an active Paddle subscription."""
    from payments.paddle import get_subscription_status
    status = await get_subscription_status(user_id)
    return status


@app.post('/webhooks/paddle', summary='Paddle webhook receiver')
async def paddle_webhook(request: Request):
    """
    Receives Paddle webhook events.
    Processes: subscription.created, subscription.canceled, transaction.completed
    """
    from payments.paddle import verify_webhook_signature, parse_webhook_event
    ...
    if event:
        # Log it (Sentry + print)
        print(f"[Paddle webhook] {event['type']} for user {event.get('user_id')}")
        try:
            import sentry_sdk as _sentry
            _sentry.capture_message(f"Paddle event: {event['type']}", level='info')
        except Exception:
            pass
...
# ---------------------------------------------------------------------------
# GET /solar-return — Birthday year-ahead reading
# ---------------------------------------------------------------------------

@app.get('/solar-return', summary="Get this year's Solar Return reading")
async def solar_return(
    user_id: str = Depends(get_current_user_id),
    db: AsyncSession = Depends(get_db),
):
    """
    Returns a full year-ahead reading based on the user's Solar Return chart.
    The Solar Return is the moment the Sun returns to its exact natal position each year.
    Cached for the year — only recalculated on the user's birthday.
    """
    from datetime import date
    user = await get_user_by_id(db, user_id)
    if not user:
    ...
    cache_key = f"solray_solar_return_{current_year}"
    cached = await get_cached_forecast(db, user_id, f"solar_return_{current_year}")
    if cached:
        return cached
    ...
    try:
        ...
        # Find this year's solar return date
        birth_date = user.birth_date
        birth_time = user.birth_time
        birth_lat = user.birth_lat or 0
        birth_lon = user.birth_lon or 0

        dt = datetime.strptime(f"{birth_date} {birth_time}", "%Y-%m-%d %H:%M")
        jd_birth = swe.julday(dt.year, dt.month, dt.day, dt.hour + dt.minute / 60.0)
        natal_sun, _ = swe.calc_ut(jd_birth, swe.SUN, swe.FLG_MOSEPH)
        natal_sun_lon = natal_sun[0]

        # Find when Sun returns to natal position this year
        jd_approx = swe.julday(current_year, dt.month, dt.day, 12.0)
        jd_lo, jd_hi = jd_approx - 2, jd_approx + 2
        for _ in range(50):
            jd_mid = (jd_lo + jd_hi) / 2
            sun_now, _ = swe.calc_ut(jd_mid, swe.SUN, swe.FLG_MOSEPH)
            diff = ((sun_now[0] - natal_sun_lon + 180) % 360) - 180
            if abs(diff) < 0.00001:
                break
            if diff > 0:
                jd_hi = jd_mid
        ...
        sr_date = swe.revjul(jd_mid)
        sr_date_str = f"{int(sr_date[0])}-{int(sr_date[1]):02d}-{int(sr_date[2]):02d}"

        # Generate AI reading for the year ahead
        ...
        sun_sign = natal_planets.get('Sun', {}).get('sign', '?')
        ...
        response = client.messages.create(
            ...
            messages=[{
                "role": "user",
                "content": f"""Write a Solar Return reading for this person's {current_year} birthday year.
Their Solar Return falls on {sr_date_str}. The Sun is returning to its natal position in {sun_sign}.

Write:
1. A title for this year (1 sentence, poetic, specific to their chart)
2. The dominant theme of this year (2-3 sentences grounded in their placements)
3. Three areas of life that will be activated (each 2 sentences)
4. One key challenge and how their chart gives them the tools to meet it
5. A closing question in italics

Speak directly from their chart. No generic birthday readings."""
            }]
        )
        ...
        result = {
            'year': current_year,
            'solar_return_date': sr_date_str,
            'reading': reading,
            'sun_sign': sun_sign,
        }

        # Cache for the year
        await cache_forecast(db, user_id, f"solar_return_{current_year}", result)
        return result

    except Exception as e:
        raise HTTPException(status_code=500, detail=f'Solar Return calculation failed: {str(e)}')
...
async def push_subscribe(
    ...
    """Store a Web Push subscription for sending transit alerts."""
    import uuid
